...
A partir desse levantamento de dados, foi possível determinar o escopo do software e hardware para o projeto, os quais são explicados de maneira mais rica nos capítulos a seguir.
...
O  ESP32 Espressif foi escolhido pois atende às necessidades do projeto, atendendo aos requisitos como número de pinos I/O, quantidade de armazenamento e comunicação sem fio WiFi. Além disso, pode ser obtido a um baixo custo e apresenta uma boa documentação.
...
\begin{itemize}
    ...
    \item A segunda é que, diferente das versões que geram uma tensão alternada proporcional à corrente sendo medida, que essa variação do sensor gera uma corrente alternada, essa corrente precisa ser processada por meio de um circuito de condicionamento, o qual atua transformando a corrente alternada em uma tensão que pode ser lida e processada pelo microcontrolador.
\end{itemize}
%O sensor YF-S201 é acessível e, por meio de experimentos, demonstrou operar adequadamente com 3,3 V
O sensor de vazão de água YF-S201 é acessível a um preço baixo e, por meio de experimentos, demonstrou operar adequadamente com 3,3 V, o que é mais baixo que sua tensão nominal de operação de 5 V. Isso é relevante pois simplifica seu uso com o microcontrolador escolhido, o ESP32, cujos pinos suportam somente tensões até 3,6 V.
...
A caixa para o circuito elétrico, para o LCD e o botão podem ser fixados na parede do banheiro próximo ao chuveiro como na Figura~\ref{fig:Sistema_montado}. 
...
Com o hardware consolidado, o desenvolvimento do software, além da funcionalidade, deve priorizar a experiência do usuário, gerando uma interface amigável e intuitiva. A implementação técnica foi executada na IDE Visual Studio Code, através do plugin PlatformIO, e ocorreu em duas frentes, firmware e o aplicativo web.

\subsection{Firmware}

O firmware foi escrito em C++, é responsável por toda a lógica de detecção de banhos, leitura, gerenciamento e persistência dos dados. Seu funcionamento é ilustrado na Figura~\ref{fig:firmware_flowchart}.
...
\begin{figure}[htbp]%% Ambiente figure
 ...
 \end{figure}
\FloatBarrier
%A arquitetura de software, desenvolvida para o microcontrolador ESP32, organiza-se em um loop principal que realiza a aquisição contínua de dados provenientes de sensores de corrente elétrica e vazão de água. A lógica de detecção de eventos utiliza limiares configuráveis para identificar o início e término de sessões de banho, empregando uma máquina de estados finitos que distingue entre estados ativos e inativos. Durante a operação ativa, o sistema realiza integração temporal dos sinais medidos para quantificar o consumo total de água em litros e energia em quilowatts-hora, aplicando fatores de conversão monetária para apresentação em tempo real através de interface LCD. Os dados históricos são persistidos em sistema de arquivos LittleFS no formato CSV, permitindo análises estatísticas e cálculo de médias de consumo. O sistema incorpora ainda funcionalidades de conectividade WiFi para acesso remoto via servidor web assíncrono, além de mecanismos de reset automático programável e recuperação de configurações através de memória não-volátil, garantindo operação autônoma e confiável.

O sistema continuamente monitora os sensores de vazão e corrente, e, ao detectar um banho, quantifica os sinais recebidos dos sensores em litros de água e em quilowatts-hora de energia, calculando a conversão monetária para apresentação em tempo real através da interface LCD e pelo Web App. Informações importantes como configurações e os dados históricos dos banhos são armazenadas na memória flash interna do microcontrolador, garantindo persistência mesmo em caso de reinicializações ou interrupções de energia.

Um sistema de reiniciamento automático foi implementado para assegurar uma operação autônoma e confiável.


\subsection{Web App}
 A solução web app foi escolhida porque, ao contrário de um aplicativo instalado, ela pode ser acessada em qualquer dispositivo com suporte a um navegador moderno, sem a necessidade de instalar softwares adicionais. Isso torna o acesso mais fácil para os usuários e simplifica o processo de desenvolvimento.
 
 Foi desenvolvido como método primário de interagir com o projeto, fornecendo tanto um feedback em tempo real quanto o acesso aos dados coletados ao longo do tempo. A Figura~\ref{fig:Web_Mockup} ilustra o protótipo de interface que orientou o processo de desenvolvimento.
...
A interface web foi implementada com tecnologias web padrão, HTML, CSS e JavaScript, de forma semelhante a um site tradicional. O uso de JavaScript permite o processamento dos dados recebidos diretamente no navegador, aliviando a carga computacional do microcontrolador, que já é responsável pela análise contínua dos sensores.
 ...
 No final da página temos dois botões, o da esquerda força a atualização da página, e o da direita abre a aba de configuração.
...
\begin{figure}[htbp]%% Ambiente figure
 ...
 \begin{itemize}
     \item No topo temos o gráfico com os custos dos banhos do dia.
     \item No meio temos o gráfico com os custos de todos os banhos das últimas duas semanas.
...
No final da página temos uma aba que mostra, de maneira menos ilustrada, porém mais detalhada, uma tabela com todos os banhos tomados que estão guardados na memória do dispositivo. Aqui essa tabela pode sei baixada na forma de um arquivo csv.
...
Alicate amperímetro na Figura~\ref{fig:amperimetro}.
...
\begin{itemize}
    ...
    \item $V_l$ é o volume de água medido;
    \item $t$ é o intervalo de tempo no qual o volume de água foi coletado.
\end{itemize}
...
É notável, pelos valores da terceira coluna da Tabela~\ref{tab:tabVazao}, que o desempenho do sensor de vazão degrada significativamente em condições de baixo fluxo de água.

Por fim, o desempenho geral do sistema mostrou-se aceitável para a aplicação proposta. 

O sensor de corrente SCT-013 demonstrou consistência ao longo das medições, com desvios máximos de 2,53\% em relação ao valor esperado. 
Já o medidor de vazão YF-S201 demonstrou incapaz de obter leituras inferiores a 2 L/min sem um erro significativo. Entretando, de acordo com o \textit{Brazil Water Efficiency Report} \cite{clasp2021water}, a maior parte dos chuveiros elétricos comercializados no país operam com vazão entre 3 e 6 L/min, ou seja, o erro do sensor não se apresenta de maneira significativa dentro da faixa de operação esperada do sistema, não comprometendo a aplicabilidade do mesmo.

\section{Análise dos Dados Coletados pelo Sistema}

O sistema foi monitorado por duas semanas, como ilustrada pelo gráfico na Figura~\ref{fig:WebPrimeirasDuasSemanas}. Nos primeiros dias, eram gastos, por dia, entorno de R\$7 reais com banhos. Após o usuário se acostumar com o feedback em tempo real, houve uma tendência de queda dos gastos, que caíram para R\$5 por dia, demonstrando a eficácia do sistema em promover a conscientização.
